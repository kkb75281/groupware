export function createTable(cols, rows, isCreate = false) {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      colResizer.className = 'table-resizer col-resizer';
      colResizer.setAttribute('data-col-index', c);
      td.appendChild(colResizer);

      const rowResizer = document.createElement('div');
      rowResizer.className = 'table-resizer row-resizer';
      rowResizer.setAttribute('data-row-index', c);
  ...
  function selectCellsInRange(start, end) {
    ...
    for (let r = minRow; r <= maxRow; r++) {
      ...
      for (let c = minCol; c <= maxCol; c++) {
        if (cells[c]) {
          if (!cells[c]) continue;
          //   if (window.getComputedStyle(cells[c]).display === 'none') continue;
          if (!cells[c].offsetParent) {
            console.log('보이지 않는 셀 건너뜀: ', { r, c });
            continue;
          }
