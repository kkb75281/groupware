...
export const createTable = (col, row) => {
  ...
  table.getRowsCount = () => table.row.length; // 테이블의 행 수
  table.getColsCount = () => (table.row[0] ? table.row[0].cells.length : 0); // 테이블의 열 수
  ...
  table.removeColumn = (colIndex) => {
    let row = table.getRowsCount();
    for (let r = 0; r < row; r++) {
      if (colIndex >= 0 && colIndex < table.row[r].cells.length) {
        table.row[r].deleteCell(colIndex);
      }
    }
  };
