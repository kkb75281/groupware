		...
		.modal-body
			.select-dvs-wrap
				.organigram-wrap
					...
					Organigram(:useCheckbox="true" :selectedEmployees="selectedEmps" @selection-change="handleOrganigramSelection")
...
const handleOrganigramSelection = (users) => {
	users.forEach((user) => {
		...
		const newUserInfo = {
			...
			dvs: `${user.division}.${divisionNameList.value[division]}`,
		...
		Object.keys(selectedDivision.value).forEach((division) => {
			...
			departmentUsers.forEach((user) => {
				...
				const isDuplicate = selectedEmps.value.some(
					(existingUser) => existingUser.user?.user_id === userCopy.user_id
				);
...
const registerNewsCat = async () => {
	...
	const accessUserId = selectedEmps.value.map((user) => user.user.user_id).filter(Boolean);
