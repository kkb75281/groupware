                            ...
                            tbody
                                ...
                                tr.approval(v-if="selectedAuditors.approvers.length > 0")
                                    ...
                                    td.left(colspan="3" style="padding: 0; height: 119px;")
                                        ul.approver-wrap
                                            li.approver-list(v-for="(approver, index) in selectedAuditors.approvers" :key="approver?.user?.user_id")
                                                span.num {{ approver.order }}
                                                span.approver {{ approver?.user?.name }}
                                ...
                                tr.approval(v-if="selectedAuditors.agreers.length > 0")
                                    ...
                                    td.left(colspan="3" style="padding: 0; height: 119px;")
                                        ul.approver-wrap
                                            li.approver-list(v-for="(agreer, index) in selectedAuditors.agreers" :key="agreer?.user?.user_id")
                                                span.num {{ agreer.order }}
                                                span.approver {{ agreer?.user?.name }}
                                ...
                                tr.reference(v-if="selectedAuditors.receivers.length > 0")
                                    ...
                                    td.left(colspan="3")
                                        ul.reference-wrap
                                            li.reference-list(v-for="(receiver, index) in selectedAuditors.receivers" :key="receiver?.user?.user_id")
                                                span.referencer {{ receiver?.user?.name }}
                ...
                .table-wrap
                    .tb-overflow(v-if="selectedUsers.length > 0")
                        table.table#selected_auditors
                            ...
                            tbody
                                tr(v-for="(user, index) in selectedUsers" :key="user.user.user_id")
                                    td
                        ...
                        table.table#tb-referList
                            ...
                            thead
                                tr
                                    ...
                                    th 기안자
                            tbody
                                tr(v-for="(doc, index) in referDocDisplayList" :key="doc.record_id")
                                    td
                                        label.checkbox
                                    ...
                                    td {{ doc.drafter }}
                                tr(v-if="referDocDisplayList.length === 0 && !referDocFetching")
                                    td(colspan="4")
                                        span.empty 선택할 참조 문서가 없습니다.

                .pagination(v-if="referDocMaxPage > 1")
                    button.btn-prev.icon(type="button" @click="referDocCurrentPage--" 
                        :class="{'nonClickable': referDocFetching || referDocCurrentPage <= 1}")
                        | Prev
                    
                    span.page-info {{ referDocCurrentPage }} / {{ referDocMaxPage }}
                    
                    button.btn-next.icon(type="button" @click="referDocCurrentPage++" 
                        :class="{'nonClickable': referDocFetching || referDocEndOfList && referDocCurrentPage >= referDocMaxPage}")
                        | Next
...
// 참조문서 페이지네이션
let referDocPager = null;
const referDocFetching = ref(false);
const referDocMaxPage = ref(0);
const referDocCurrentPage = ref(1);
const referDocEndOfList = ref(false);
const referDocDisplayList = ref([]);
...
const checkRole = (user) => {
    ...
    const index = selectedUsers.value.findIndex((u) => u.user.user_id === user.user.user_id);
...
const postAuditDoc = async ({ docform_title, to_audit, to_audit_content }) => {
    ...
    const send_auditors_data = {
        approvers: selectedAuditors.value.approvers.map((user) => ({
            user_id: user.user.user_id.replaceAll('-', '_'),
            order: user.order
        })),
        ...
        const res = await Promise.all(
            processRoles.map((roleInfo, index) =>
                postAuditDocRecordId(
                    ...
                    roleInfo.role,
                    // 알림 대상인지 여부 확인
                    notificationTargets.some((target) => target.user.user_id === roleInfo.userId)
                )
            )
        );
...
// 양식 선택시 데이터 적용 공통 함수
const applyFormData = async (formData) => {
    ...
    if (formData.data.auditors) {
        ...
        // 비동기 작업 병렬 실행
        selectedAuditors.value = {
            approvers: await convert(auditors.approvers || [], 'approvers'),
            agreers: await convert(auditors.agreers || [], 'agreers'),
            receivers: await convert(auditors.receivers || [], 'receivers')
        };
...
const moveUser = (user, direction) => {
    // 현재 선택된 사용자의 인덱스 찾기
    const currentIndex = selectedUsers.value.findIndex((u) => u.user.user_id === user.user.user_id);

    const isUp = direction === 'up';
    const targetIndex = isUp ? currentIndex - 1 : currentIndex + 1;

    // 이동 불가능한 경우
    if (targetIndex < 0 || targetIndex >= selectedUsers.value.length) return;

    const currentRole = selectedUsers.value[currentIndex].role;
    const targetRole = selectedUsers.value[targetIndex].role;

    // 이동 제한 조건
    if (
        (isUp && currentRole === 'receivers') || // 위로 이동 시 현재 항목이 receivers인 경우
        (isUp && targetRole === 'receivers') || // 위로 이동 시 바로 위 항목이 receivers인 경우
        (!isUp && currentRole !== 'receivers' && targetRole === 'receivers') // 아래로 이동 시 현재 항목이 receivers가 아니고 바로 아래 항목이 receivers인 경우
    ) {
        return; // 이동 불가능
    }
    ...
    // 현재 항목과 타겟 항목 스왑
    [newSelectedUsers[currentIndex], newSelectedUsers[targetIndex]] = [
        newSelectedUsers[targetIndex],
        newSelectedUsers[currentIndex]
    ];

    selectedUsers.value = newSelectedUsers; // 변경된 배열 저장
    reorderUsers(); // 순서 재할당
};
...
const openReferModal = async () => {
    ...
    referDocCurrentPage.value = 1; // 페이지 초기화
    document.body.style.overflow = 'hidden';

    // 페이저 초기화
    await getReferDocPage(true);
    console.log('referDocFilter.value : ', referDocFilter.value);
    loading.value = false;
};

// 참조문서추가 모달 페이지네이션
const getReferDocPage = async (refresh = false) => {
    if (refresh) {
        referDocEndOfList.value = false;
    }

    // 새로고침 경우에만 페이저 초기화
    if (refresh || !referDocPager) {
        referDocPager = await Pager.init({
            id: 'record_id',
            resultsPerPage: 10,
            sortBy: 'uploaded',
            order: 'desc'
    ...
    // 이미 로드된 페이지이거나 더 이상 데이터가 없으면 페이저에서 가겨오기
    if (
        (!refresh && referDocMaxPage.value >= referDocCurrentPage.value) ||
        referDocEndOfList.value
    ) {
        const pageData = referDocPager.getPage(referDocCurrentPage.value);
        referDocDisplayList.value = pageData.list;
        referDocMaxPage.value = pageData.maxPage;
        return;
    }

    // 서버에서 새 데이터 가져오기
    if (!referDocEndOfList.value || refresh) {
        referDocFetching.value = true;
        ...
        try {
            ...
            // 더 이상 새 데이터가 없으면 endOfList 설정
            referDocEndOfList.value = allDocs.length < fetchOptions.limit;

            // 페이저에 데이터 삽입
            if (allDocs.length > 0) {
                await referDocPager.value.insertItems(allDocs);
            }

            // 현재 페이지 데이터 가져오기
            const pageData = referDocPager.value.getPage(referDocCurrentPage.value);
            referDocDisplayList.value = pageData.list;
            referDocMaxPage.value = pageData.maxPage;
        } catch (error) {
            console.error('참조문서 목록 가져오기 중 오류:', error);
            referDocDisplayList.value = [];
        } finally {
            referDocFetching.value = false;
        }
    }
...
// 페이지 변경 감지
watch(referDocCurrentPage, (newPage, oldPage) => {
    if (
        newPage !== oldPage &&
        newPage > 0 &&
        (newPage <= referDocMaxPage.value ||
            (newPage > referDocMaxPage.value && !referDocEndOfList.value))
    ) {
        getReferDocPage();
    } else {
        referDocCurrentPage.value = oldPage;
    }
});

watch(referDocFilter, () => {
    // 필터 변경 시 페이지 리셋하고 다시 로드
    referDocCurrentPage.value = 1;
    getReferDocPage(true);
});
