...
const updateDesc = (record) => {
	...
	if (recordIndex !== -1) {
		...
		// 로컬스토리지에 변경 사항 저장
		window.localStorage.setItem(`commuteRecords : ${user.user_id}`, JSON.stringify(commuteRecords.value));
	}
};
...
const startWork = () => {
  ...
  commuteStorage.push({ ...value });	// 출근 이력 저장소에 추가
  commuteRecords.value = commuteStorage;

  timeRecords.value = value;

  window.localStorage.setItem(`commuteRecords : ${user.user_id}`, JSON.stringify(commuteStorage));
};
...
const endWork = () => {
  ...
  if(!value || value === undefined) {
		...
		commuteStorage.push({ ...newCommuteData });	// 출근 이력 저장소에 추가
		commuteRecords.value = commuteStorage;
  ...
  if (!isEndWork) {
    const maxTimeStr = new Date(maxEndTime).toLocaleTimeString();
		...
		if(newEndWork) {
			...
			commuteStorage = [...commuteStorage, data];	// 저장소 맨 뒤에 data 추가
			commuteRecords.value = commuteStorage;
  ...
  const notLastCommutes = commuteStorage.slice(0, commuteStorage.length - 1);	// 마지막 이력 제외
...
onMounted(async () => {
  ...
  commuteStorage = JSON.parse(window.localStorage.getItem(`commuteRecords : ${user.user_id}`)) || [];
  onRecord();

	if (commuteStorage.length > 0) {
    monthlyWorkTime.value = convertMsToTime(parseInt(commuteStorage[commuteStorage.length - 1].totalCommuteTime, 10));  // 총 근무시간
  }
});
</script>
