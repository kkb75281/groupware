...
export async function getDivisionData(refresh: boolean = false) {
  if (!refresh && getDivisionDataRunning instanceof Promise) {
    // 이미 실행중인 경우
    await getDivisionDataRunning;
    return divisions.value;
  }

  if (Object.keys(divisions.value).length && !refresh) {
    // 받아온적 없거나, 데이터가 없는경우 + 새로고침을 요청하지 않은은 경우
    loading.value = false;
